...
const kedua: React.FC = () => {
    ...
    let level = 'Level Inferior',
        code = generateRandomCode("IF"),
        emoji = '😊',
        title = 'Selesai!';
    if (foundCount >= 3 && foundCount <= 5) {
        level = 'Level Reguler';
        code = generateRandomCode("RG");
        emoji = '👍';
        title = 'Bagus!';
    }
    if (foundCount === 6) {
        level = 'Level Superior';
        code = generateRandomCode("SP");
        emoji = '🎉';
        title = 'Luar Biasa!';
    }
                ...
                {gridData.map((row, r) => (
                    <div key={r} className="flex">
                        {row.map((val, c) => (
                            <div
                                key={c}
                                className={`cell flex h-14 w-14 items-center justify-center border-2 border-gray-300 text-base text-gray-800 font-bold sm:text-lg ${val === null ? 'empty' : 'cursor-pointer bg-white'} ${selectedCells.includes(cellKey(r, c)) ? 'selected' : ''} ${getMatchClass(r, c)} `}
                                onClick={() =>
